...
/**
 * Returns true if a model ID is vision-capable (can understand images).
 * This includes models like gemini-1.5-pro, gemini-1.5-flash, etc.
 *
 * @param {string} modelId
 * @returns {boolean}
 */
export function isVisionCapableModel(modelId) {
  ...
  if (id.includes('gemini-1.5-pro')) return true;
  if (id.includes('gemini-1.5-flash')) return true;
  if (id.includes('gemini-pro-vision')) return true; // Note: marked as deprecated but still vision-capable
  if (id.includes('gemini-1.0-pro-vision')) return true; // Also deprecated but vision-capable
...
/**
 * Filters out unstable/preview models and sorts vision-capable models latest-first.
 *
 * @param {string[]} modelIds
 * @returns {string[]}
 */
export function filterAndSortVisionModels(modelIds) {
  if (!Array.isArray(modelIds)) return [];
  // Filter out unstable models, then keep only vision-capable models
  const stableModels = modelIds.filter((id) => !isUnstableModel(id));
  const visionModels = stableModels.filter((id) => isVisionCapableModel(id));
  return sortModelsLatestFirst(visionModels);
}
