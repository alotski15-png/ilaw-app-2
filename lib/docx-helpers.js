...
export const buildSignatoriesTable = async ({ teacherSignatory, masterTeacherSignatory, principalSignatory }) => {
  ...
  return new Table({
    ...
    rows: [
      new TableRow({
        children: [
          await createCell(`Prepared by:\n\n\n${teacherSignatory.name || ''}\n${teacherSignatory.designation || ''}`, { widthPct: 33, borderColor: TEMPLATE_BORDER_COLOR_HEADER, cellMargins: TEMPLATE_HEADER_CELL_MARGINS, bold: true, labelSize: 28, fontSize: 20 }),
          await createCell(`Checked and Reviewed:\n\n\n${masterTeacherSignatory.name || ''}\n${masterTeacherSignatory.designation || ''}`, { widthPct: 33, borderColor: TEMPLATE_BORDER_COLOR_HEADER, cellMargins: TEMPLATE_HEADER_CELL_MARGINS, bold: true, labelSize: 28, fontSize: 20 }),
          await createCell(`Noted by:\n\n\n${principalSignatory.name || ''}\n${principalSignatory.designation || ''}`, { widthPct: 34, borderColor: TEMPLATE_BORDER_COLOR_HEADER, cellMargins: TEMPLATE_HEADER_CELL_MARGINS, bold: true, labelSize: 28, fontSize: 20 })
        ]
      })
    ]
  });
    ...
    rows: [
      ...
      new TableRow({
        children: [
          ...
          await createCell(formatDocxText(lessonPlan.header?.references || snapshotData?.references), { colSpan: 3, widthMm: 144.8, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 20 })
        ]
      }),
      new TableRow({
        children: [
          ...
          await createCell(formatDocxText(lessonPlan.header?.declarationOfAiUse || `Consistent with policy guidelines on AI in basic education...`), { colSpan: 3, widthMm: 144.8, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 20 })
        ]
      }),
      ...
      new TableRow({
        children: [
          await createCell('Opportunities for Integration and Contextualization:\nWrite down any possibilities to meaningfully connect to another learning area, special topic, local context, or technology. Write N/A if none.', { bold: true, widthMm: 45.2, labelSize: 28, fontSize: 20, borderColor: matrixBorder, cellMargins: matrixMargins }),
