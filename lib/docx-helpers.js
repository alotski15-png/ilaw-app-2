...
export const createCell = async (text, options = {}) => {
  ...
  if (Array.isArray(text)) {
    ...
    // It's a string, split by newlines and parse **bold** markers
    // **(Indicator X.X.X)** annotations are rendered as bold red text
    paragraphs = String(text || '').split('\n').map(line => {
      // Parse **bold** segments in the line
      const parts = line.split(/(\*\*.*?\*\*)/g);
      const children = parts.filter(p => p !== '').map(part => {
        if (part.startsWith('**') && part.endsWith('**')) {
          const innerText = part.slice(2, -2);
          // Check if this is an Indicator annotation
          const isIndicator = innerText.startsWith('(Indicator');
          return new TextRun({
            text: innerText,
            bold: true,
            italic: !!italic,
            color: isIndicator ? 'FF0000' : color,
            size: fontSize,
            font: 'Aptos',
          });
        }
...
export const buildSessionTable = async ({ lessonPlan, snapshotData, sessionIndex, sessionLabel, headerBannerStyle = {} }) => {
  ...
  const sessionTable = new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [await createCell('Learning Area:', { bold: true, widthPct: 25, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 }), await createCell(formatDocxText(lessonPlan.header?.learningArea || snapshotData?.subject), { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })] }),
      new TableRow({ children: [await createCell('Name of Teachers:', { bold: true, widthPct: 25, borderColor: matrixBorder, cellMargins: matrixMargins }), await createCell(formatDocxText(getOnlyName(lessonPlan.header?.teacherName || snapshotData?.teacherName)), { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })] }),
      new TableRow({ children: [await createCell('Grade level & Section:', { bold: true, widthPct: 25, borderColor: matrixBorder, cellMargins: matrixMargins }), await createCell(formatDocxText(lessonPlan.header?.gradeLevelSection || snapshotData?.gradeAndSection), { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })] }),
      new TableRow({ children: [await createCell('No. of Sessions:', { bold: true, widthPct: 25, borderColor: matrixBorder, cellMargins: matrixMargins }), await createCell(`${snapshotData?.noOfSessions || ''} (${sessionLabel})`, { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })] }),
      new TableRow({
        children: [
          await createCell([
          ...
          await createCell(formatDocxText(lessonPlan.header?.references || snapshotData?.references), { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
      new TableRow({
        children: [
          ...
          await createCell(formatDocxText(lessonPlan.header?.declarationOfAiUse || `Consistent with policy guidelines on AI in basic education...`), { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
      new TableRow({ children: [await createCell('Intentions:', { ...headerBannerStyle, widthPct: 25, borderColor: matrixBorder, cellMargins: matrixMargins }), await createCell('Meaningful learning experiences are anchored in how we frame them. Start by deciding what you want learners to master by the end of the lesson \u2013 keep it clear and simple.\nRemember: Understanding your learner\u2019s evolving context and designing around that your lessons connect with and are relevant to them.', { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })] }),
      new TableRow({
        children: [
          await createCell([
          ...
          await createCell(`**Content Standard:**\n${formatDocxText(lessonPlan.curriculumStandards?.contentStandard || snapshotData?.contentStandards)}\n\n**Performance Standard:**\n${formatDocxText(lessonPlan.curriculumStandards?.performanceStandard || snapshotData?.performanceStandards)}\n\n**Learning Competency:**\n${formatDocxText(lessonPlan.curriculumStandards?.learningCompetency || snapshotData?.learningCompetency)}`, { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
      new TableRow({
        children: [
          ...
          await createCell(objectives, { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
      new TableRow({
        children: [
      ...
      new TableRow({
        children: [
          ...
          await createCell(preLesson, { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
      new TableRow({
        children: [
          ...
          await createCell(flow, { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
      new TableRow({
        children: [
          ...
          await createCell(resources, { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
      new TableRow({
        children: [
      ...
      new TableRow({
        children: [
          ...
          await createCell(assessment, { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
      new TableRow({ children: [await createCell('Ways Forward', { ...headerBannerStyle, widthPct: 25, borderColor: matrixBorder, cellMargins: matrixMargins }), await createCell('Meaningful learning can also happen beyond the classroom \u2013 for both the learners and the teacher.\nPause and reflect on what happened today.', { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })] }),
      new TableRow({
        children: [
          await createCell([
          ...
          await createCell(extended, { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
            new TableRow({
        height: { value: 2835, rule: 'exact' },
        children: [
          await createCell('Reflections', { bold: true, widthPct: 25, borderColor: matrixBorder, cellMargins: matrixMargins }),
          await createCell('', { colSpan: 3, widthPct: 75, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })
        ]
      }),
    ]
  });
