...
export const buildSessionTable = async ({ lessonPlan, snapshotData, sessionIndex, sessionLabel, headerBannerStyle = {} }) => {
  ...
  const sessionTable = new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [await createCell('Ways Forward', { ...headerBannerStyle, bold: true, widthMm: 45.2, labelSize: 28, fontSize: 20, borderColor: matrixBorder, cellMargins: matrixMargins }), await createCell('Meaningful learning can also happen beyond the classroom — for both the learners and the teacher.\nPause and reflect on what happened today.', { colSpan: 3, widthMm: 144.8, borderColor: matrixBorder, cellMargins: matrixMargins, fontSize: 24 })] }),
      new TableRow({
        children: [
        ...
        children: [
          await createCell('Reflections', { bold: true, widthMm: 45.2, labelSize: 28, fontSize: 20, borderColor: matrixBorder, cellMargins: matrixMargins }),
